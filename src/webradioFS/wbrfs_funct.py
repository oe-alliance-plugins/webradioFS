...
from Components.config import ConfigYesNo, ConfigInteger, ConfigSelection, NoSave
...
from enigma import eListboxPythonMultiContent, RT_HALIGN_LEFT, RT_VALIGN_CENTER, eListboxPythonConfigContent  # , getDesktop
from enigma import eConsoleAppContainer, eWindowStyleSkinned, eWindowStyleManager
import os
       ...
       if anfrage:
            ...
            for row in cursor:
                  ...
                  if len(sets) > 2 and sets[2].strip() != "None":
                      art = sets[2]
                  repeat = "daily"
                  if len(sets) > 3 and sets[3].strip() != "None":
                     repeat = sets[3]
                  datum = None
...
class Streamlist:
    ...
    def streams(session, stream=None, typ=None, timeout1=10, etest=None):
        ...
        typ_check = 'mp3'
        status = 200  # noqa F841
        reason = ''  # noqa F841
        htyp = ''  # noqa F841
        t_url = None  # noqa F841
        conn = None  # noqa F841
        typ7 = None
        typ1 = typ
        if stream:
            ...
            try:
                ...
                if fontlist[4]:
                    read_timer_conn = read_timer.timeout.connect(path.close)  # noqa F841
                else:
                    read_timer.callback.append(path.close)
                p1 = path.info().get('content-type')  # noqa F841
                p2 = path.geturl()
                if p2:
                    stream = p2
                #f7=open("/tmp/001","a")
                #p1.json()
                #out = path.info().json()
                out = dict(path.info())  # noqa F841
                #f7.write(str(out)+"\n")
                #f7.write(str(path.info())+"\n")
                #f7.close()
                i1 = dict(path.info())

                if 'content-type' in i1 and etest is None:
    ...
    def start(self, text='unknown', titel='webradioFS', aktion=None, default=False, type1='INFO', timeout=0, sd=3, fonts1=None):
        ...
        for x in self.liste:
                 ...
                 if x and len(x):
                     ...
                     if txt:
                        m_res.append(MultiContentEntryText(pos=(10, 5), size=(breit - 60, self.fonts[2]), font=0, text=txt, color=0xffffff, backcolor=0x8B6508, color_sel=0x000000, backcolor_sel=0xffffff, flags=RT_HALIGN_LEFT | RT_VALIGN_CENTER))  #,color=0x000000,backcolor=0x00008B,color_sel=0xf47d19,backcolor_sel = 0x000000
                 i_res.append(m_res)
        if len(i_res):
           self.mListe.setList(i_res)
        if type1 == '??':
        ...
        if self.num and self.settigspath:
            ...
            if pathExists(mpath) is True:
                ...
                if tarfile:
                    ...
                    try:
                        tarFiles = ' '.join(files)
                        lines = popen('tar cv%sf %s %s' % (comp, self.settigspath, tarFiles)).readlines()  # noqa F841
    ...
    def callOverwriteSettings(self, res=None):
        if res:
                ...
                if tarfile:
                    ...
                    try:
                        lines = popen('tar xv%sf %s -C /' % (comp, self.settigspath)).readlines()  # noqa F841
                        self.session.open(MessageBox, _('Settings were restored successfully') + "\n" + _('Please restart webradioFS'), type=MessageBox.TYPE_INFO, timeout=15)
                    except Exception as e:
                        self.session.open(MessageBox, _('Restore-Error:') + str(e), type=MessageBox.TYPE_ERROR, timeout=15)
...
class webradioFSdisplay13(Screen):
    def __init__(self, session, parent):
        ...
        lcd_sets = reads[0].split(',')  # noqa F841
        sets = reads[1].split(',')
        self.pic_set = []
        self.pic_what = 1
        ...
        z_list = ['Zeile1', 'Zeile2', 'Zeile3']  # noqa F841
        if self.art == 0:
            self.label_list = []
            self.label_pos = []
            num = 1
...
class webradioFSsetDisplay(Screen, ConfigListScreen):
    def __init__(self, session, fonts=None):
        ...
        if int(self.m_sets["art"]) > 2:
            ...
            if r11 not in self.choices_test:
               r11 = ''
            if r12 not in self.choices_test:
               r12 = ''
            if r13 not in self.choices_test:
               r13 = ''
        else:
    ...
    def refresh(self):
        if self.art.value == 3:
            if self.l1t.value not in self.choices_test:
               self.l1t.value = ''
            if self.l2t.value not in self.choices_test:
               self.l2t.value = ''
            if self.l3t.value not in self.choices_test:
               self.l3t.value = ''
        ...
        if self.art.value == 2:
            ...
            if self.art.value == 0:
                timDisplConfigList.extend((getConfigListEntry(_('Display-Len:') + ' (0-999)', self.len1),
                                           ...
                                           getConfigListEntry('')))
                if self.pic_on.value is not False:
    ...
    def saving(self, save_list):
        if self.art.value == 3:
                if not path.exists("/etc/ConfFS/wbrFS_display.xml"):
                    ret = copyfile(plugin_path + "/skin/wbrFS_display.xml", "/etc/ConfFS/wbrFS_display.xml")  # noqa F841
        if len(save_list):
          connection = sqlite.connect(set_file)
          connection.text_factory = str
          cursor = connection.cursor()
                           ...
                           if mutagen:
                                ...
                                if maudio:
                                    ...
                                    try:
                                        ...
                                        mlength = str(timedelta(seconds=mlen)).encode("utf-8", 'ignore')
                                        if mlen < 3600:
                                            mlength = mlength[-5:]
                                    except Exception:
                                        mlength = 0
...
class fav_import:
    def imp2(self, t=None, version=None, php_num=None, num=None):
        ...
        sets_prog = startsets[0]  # noqa F841
        z = None
        from sqlite3 import dbapi2 as sqlite
        db_exists = False
    ...
    def imp2(self, dats=None):
        ...
        if liste and len(liste):
            ...
            for row in cursor:
                if str(row[1]) == n:
                    gnum = row[0]
            if gnum is not None and str(gnum) != "None":
                    ...
                    if not x.startswith("#"):
                        ...
                        if gnum and len(new) > 2:
                            if new[0] and new[1] and new[2]:
                               if len(str(new[0])):
                                  name1 = str(new[0]).strip().replace("\n", ",")
                               if len(str(new[1])):
                                  url1 = str(new[1]).strip()
                               if len(str(new[2])):
                                  typ1 = str(new[2]).strip().replace("\n", ",")
                               if len(new) > 3:
                                   if len(str(new[3])):
                                      descrip1 = str(new[3]).strip().replace("\n", ",")
                                   try:
                                       bitr = int(new[4])
                                   except Exception:
                                       pass
